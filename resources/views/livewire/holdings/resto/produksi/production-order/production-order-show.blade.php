<div>
        ...
        <div class="relative px-8 py-6 bg-emerald-600/80 rounded-b-3xl shadow-lg overflow-hidden">
            <div class="flex justify-between items-start">
                ...
                <div class="flex gap-2">
                    ...
                    @endif
                    <a href="{{ route('dashboard.resto.recipe.production') }}" class="px-3 py-1 rounded-md text-sm font-medium bg-gray-200 hover:bg-gray-300 text-gray-700">&larr; Back</a>
                </div>
            </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($order->componentPlans as $plan)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                ...
                                                @else
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800">Sub-Recipe</span>
                                                @endif
                                            </td>
                                    ...
                                    @if ($order->componentPlans->count() === 0)
                                        <tr><td colspan="9" class="py-6 text-center text-gray-400 italic">No component plans</td></tr>
                                    @endif
                                </tbody>
                            </table>
                        </div>
                        ...
                        @if (in_array($order->status, ['draft', 'issued', 'in_progress']))
                            <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-4">
                                ...
                                <form wire:submit.prevent="issueMaterial" class="grid grid-cols-4 gap-3">
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Component (Plan Line) <span class="text-red-500">*</span></label>
                                    ...
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Issue Qty <span class="text-red-500">*</span></label>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" wire:model.defer="issueForm.notes" class="w-full border-gray-300 rounded-md text-sm" placeholder="Optional">
                                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Issue Qtyd</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Unit</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    @if ($order->materialIssueLines->count() === 0)
                                        <tr><td colspan="7" class="py-6 text-center text-gray-400 italic">No material issues yet</td></tr>
                                    @endif
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="p-6">
                        @if (in_array($order->status, ['issued', 'in_progress']))
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                                <h4 class="text-sm font-semibold text-blue-800 mb-3">Record Output</h4>
                                <form wire:submit.prevent="recordOutput" class="grid grid-cols-5 gap-3">
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Output Item <span class="text-red-500">*</span></label>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    @if ($order->outputLines->count() === 0)
                                        <tr><td colspan="7" class="py-6 text-center text-gray-400 italic">No outputs yet</td></tr>
                                    @endif
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="p-6">
                        @if (in_array($order->status, ['issued', 'in_progress']))
                            <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                                <h4 class="text-sm font-semibold text-red-800 mb-3">Record Waste</h4>
                                <form wire:submit.prevent="recordWaste" class="grid grid-cols-7 gap-3">
                                    ...
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Waste Qty <span class="text-red-500">*</span></label>
                                    ...
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Notes</label>
                                        <input type="text" wire:model.defer="wasteForm.notes" class="w-full border-gray-300 rounded-md text-sm" placeholder="Optional">
                                    </div>
                                    <div class="flex items-end">
                                        <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 text-sm font-medium">Record Waste</button>
                                    </div>
                                </form>
                            </div>
                        @endif
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">#</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Item</th>
                                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Waste Qty</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Unit</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    @if ($wasteLines->count() === 0)
                                        <tr><td colspan="9" class="py-6 text-center text-gray-400 italic">Not Yet Waste</td></tr>
                                    @endif
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="p-6">
                            ...
                            <div class="space-y-4">
                                ...
                                @if ($order->status === 'in_progress')
                                    <div class="flex justify-end">
                                        <button wire:click="completeOrder" wire:confirm="Yakin ingin menyelesaikan Production Order ini? All Output akan di-post ke Inventory." class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-semibold shadow-sm">
                                            Complete Order
                                        </button>
                                    </div>
                                @endif
                        ...
                        @else
                            <div class="text-center py-12 text-gray-400">
                                <p class="text-lg font-medium">Cost Summary Not Yet Available</p>
                                <p class="text-sm mt-1">Cost Summary will be auto-generated when production order is completed.</p>
                                @if ($order->status === 'in_progress')
                                    <button wire:click="completeOrder" wire:confirm="Yakin ingin menyelesaikan Production Order ini? All Output akan di-post ke Inventory." class="mt-6 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-semibold shadow-sm">
                                        Complete Order
                                    </button>
                                @endif
                            </div>
    ...
    @else
        <div class="relative px-8 py-6 bg-emerald-600/80 rounded-b-3xl shadow-lg overflow-hidden">
            <h1 class="text-3xl font-bold text-white">Production Order No Ditemukan</h1>
            <div class="mt-4">
                <a href="{{ route('dashboard.resto.recipe.production') }}" class="px-4 py-2 bg-white text-emerald-600 rounded-md hover:bg-gray-100 text-sm font-medium">&larr; Back</a>
            </div>
        </div>
    @endif
