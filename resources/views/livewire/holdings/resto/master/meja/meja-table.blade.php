<x-ui.sccr-card transparent wire:key="meja" class="h-full min-h-0 flex flex-col">
        ...
        <div class="flex justify-between items-start">
            <div>
                ...
                <p class="text-blue-100 text-sm">
                    Master Data Table Management Resto
                </p>
            </div>
        </div>
    ...
    <div class="px-4 pt-3 pb-1">
        <div class="flex flex-wrap items-center justify-between gap-1">
            ...
            <form wire:submit.prevent="applyFilter" class="flex flex-wrap items-center gap-1 flex-grow">
                ...
                {{-- FILTER 1: Area --}}
                <div class="relative top-1">
                    <span class="absolute -top-3 left-1 text-[10px] font-bold text-black uppercase">Area</span>
                ...
                <table class="min-w-full divide-y divide-gray-900">
                    <thead class="bg-gray-700/80 text-white sticky top-0 z-10">
                        <tr>
                            ...
                            @if ($columnVisibility['table_number'])
                                <th wire:click="sortBy('table_number')"
                                    class="px-4 py-3 text-left text-xs font-bold cursor-pointer">
                                    Table No. {!! $sortField === 'table_number' ? ($sortDirection === 'asc' ? '▲' : '▼') : '↕' !!}
                                </th>
                            @endif

                            @if ($columnVisibility['capacity'])
                                <th wire:click="sortBy('capacity')"
                                    class="px-4 py-3 text-left text-xs font-bold cursor-pointer">
                                    Capacity {!! $sortField === 'capacity' ? ($sortDirection === 'asc' ? '▲' : '▼') : '↕' !!}
                                </th>
                            @endif

                            @if ($columnVisibility['area'])
                                <th class="px-4 py-3 text-left text-xs font-bold">Area</th>
                            @endif
                            ...
                            @if ($columnVisibility['notes'])
                                <th class="px-4 py-3 text-left text-xs font-bold">Notes</th>
                            @endif
    ...
    @if ($overlayMode === 'create')
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center px-6">
            <div class="w-full max-w-2xl bg-white rounded-2xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                @livewire('holdings.resto.master.meja.meja-create')
            </div>
        </div>
    @endif
    ...
    @if ($overlayMode === 'show' && $overlayId)
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center px-6">
            <div class="w-full max-w-3xl bg-white rounded-2xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                @livewire('holdings.resto.master.meja.meja-show', ['id' => $overlayId], key($overlayId))
            </div>
        </div>
    @endif
    ...
    @if ($overlayMode === 'Edit' && $overlayId)
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center px-6">
            <div class="w-full max-w-2xl bg-white rounded-2xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                @livewire('holdings.resto.master.meja.meja-Edit', ['id' => $overlayId], key($overlayId))
            </div>
        </div>
    @endif
